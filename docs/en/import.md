...
- **Transient.** LLM timeout, network blip, provider budget exhausted. The offending chunk raises `ExtractionError(fatal=False)`, the pipeline emits a `chunk.error` event, records the chunk as dropped, advances `progress.current_chunk` past it, and moves on to the next chunk. Dispatch-layer failures (DB contention, downstream exceptions) follow the same path. Failed chunks are NOT re-tried on resume — the progress cursor marks them "attempted, done" so a subsequent `resume_pipeline` call cannot re-extract the same text and duplicate any items that may have committed before the failure (audit P1-8). The operator's recovery lever is to re-import the file if they care enough about the missing chunks to replay them.
