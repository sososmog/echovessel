...
**第一次来?** **[首次启动指南](./first-time-setup.md)** 带你从 `pip install echovessel` 到在浏览器里跟第一个 persona 对话——全程大约十分钟。

急性子速览:

```bash
pip install echovessel
echovessel init         # 生成 ~/.echovessel/config.toml
echovessel run          # 首次启动会自动打开 http://localhost:7777/
```
