...
- **Transient(临时)**。LLM 超时、网络抖动、provider 预算耗尽。出事的 chunk 抛 `ExtractionError(fatal=False)`,管线发一个 `chunk.error` 事件、把它登记为 dropped item、**把 `progress.current_chunk` 推到它之后**,然后继续跑下一个 chunk。Dispatch 层失败(DB 竞用、下游异常)走同一条路径。**失败的 chunk 在 resume 时不会被重试**——进度游标把它标为"已尝试完毕",这样之后的 `resume_pipeline` 调用就不会重跑同一段文本,避免在失败前已 commit 的 items 之上再复制一份(审计 P1-8)。运营人员的恢复手段是:如果在意那几个失败 chunk 的内容,重新 import 整个文件。
